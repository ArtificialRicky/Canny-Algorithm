...
__device__ __constant__ int DY_PREV[4] = {-1, -1, -1, -1}; // adjust as in your code
...
__global__ void nms_and_double_threshold_kernel(
    float *img,              // in/out gradient magnitude
    const float *angle,      // gradient directions
    int rows, int cols,
    int step,                // in floats, not bytes
    float low_thresh,
    float high_thresh)
{
    // allocate one big shared tile per block
    __shared__ float tile[SHMEM_HEIGHT][SHMEM_WIDTH];
    ...
    // global index into img[]: row-major in floats
    int idx = i*step + j;

    // 1) load center pixel
    if (i < rows && j < cols) {
        tile[ty+1][tx+1] = img[idx];
    }

    // 2) load halo edges (only by edge threads):
    //    left/right
    if (tx==0     && j>0         && i<rows) tile[ty+1][0]              = img[i*step + (j-1)];
    if (tx==BLOCK_SIZE_X-1 && j<cols-1 && i<rows) tile[ty+1][SHMEM_WIDTH-1]  = img[i*step + (j+1)];
    //    up/down
    if (ty==0     && i>0         && j<cols) tile[0][tx+1]              = img[(i-1)*step + j];
    if (ty==BLOCK_SIZE_Y-1 && i<rows-1 && j<cols) tile[SHMEM_HEIGHT-1][tx+1] = img[(i+1)*step + j];
    //    four corners
    if (tx==0 && ty==0                 && i>0    && j>0)          tile[0][0]                  = img[(i-1)*step + (j-1)];
    if (tx==0 && ty==BLOCK_SIZE_Y-1    && i<rows-1 && j>0)       tile[SHMEM_HEIGHT-1][0]     = img[(i+1)*step + (j-1)];
    if (tx==BLOCK_SIZE_X-1 && ty==0    && i>0    && j<cols-1)     tile[0][SHMEM_WIDTH-1]      = img[(i-1)*step + (j+1)];
    if (tx==BLOCK_SIZE_X-1 && ty==BLOCK_SIZE_Y-1 && i<rows-1 && j<cols-1)
                                                          tile[SHMEM_HEIGHT-1][SHMEM_WIDTH-1] = img[(i+1)*step + (j+1)];

    __syncthreads();

    // only process true interior pixels
    if (i<1 || j<1 || i>=rows-1 || j>=cols-1) return;

    // fetch shared scratch
    float v   = tile[ty+1][tx+1];
    float ang = angle[i*cols + j];
    // quantize angle to 0–3
    int dir = ((int)roundf(ang * (4.0f / M_PI))) & 3;

    float prev = tile[ty+1 + DY_PREV[dir]][tx+1 + DX_PREV[dir]];
    float next = tile[ty+1 + DY_NEXT[dir]][tx+1 + DX_NEXT[dir]];
    ...
    float outv;
    if      (sup < low_thresh)  outv = 0.0f;
    else if (sup > high_thresh) outv = 255.0f;
    else {
        // look for any strong neighbor in 8-conn using the already-loaded tile
        ...
        for (int dy=-1; dy<=1 && !any_strong; ++dy) {
            ...
            for (int dx=-1; dx<=1; ++dx) {
                ...
                if (tile[ty+1+dy][tx+1+dx] > high_thresh) {
                    any_strong = true;
                    break;
                }
            }
        }
        outv = any_strong ? 255.0f : 0.0f;
    }
...
nms_and_double_threshold_kernel<<<grid,block>>>(
    d_mag_f, d_ang,
    rows, cols,
    cols,           // step in floats
...
cudaEventElapsedTime(&t_thr  , e1, e3);
